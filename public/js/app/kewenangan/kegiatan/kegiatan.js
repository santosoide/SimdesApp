...
app.controller('kegiatanCtrl', ['$scope', 'kegiatan', '$modal', '$stateParams', function ($scope, kegiatan, $modal) {
    ...
    $scope.getKewenangan = function () {
        ...
        kegiatan.getbidang($scope.inputData.kewenangan._id)
            .success(function (data) {
                if (data.length > 0) {
    ...
    $scope.getBidang = function () {
        ...
        kegiatan.getprogram($scope.inputData.bidang._id)
            .success(function (data2) {
                if (data2.length > 0) {
                    data2.unshift({_id: 0, program: 'Silahkan pilih sub bidang', kode_rekening: 0});
            ...
            .success(function (data) {
                kegiatan.getkewenangan()
                    .success(function (data1) {
                        ...
                        $scope.inputData.kewenangan = $scope.objKewenangan[findWithAttr($scope.objKewenangan, '_id', data.program.bidang.kewenangan._id)];
                        $scope.inputData.kewenangan_id = $scope.objKewenangan[findWithAttr($scope.objKewenangan, '_id', data.program.bidang.kewenangan._id)]._id;
                        kegiatan.getbidang($scope.inputData.kewenangan._id)
