app.controller('ModalOrganisasiCtrl', ['$scope', 'dana_desa', '$modalInstance', 'dtorganisasi', function ($scope, dana_desa, $modalInstance, dtorganisasi) {
    //$scope.ssh = ssh;
    $scope.selected = dtorganisasi;
    ...
    dana_desa.getOrganisasi($scope.main.page, $scope.main.term)
...
app.controller('modDeleteDanaDesa', ['$scope', 'dana_desa', '$modalInstance', 'id', function ($scope, dana_desa, $modalInstance, id) {
...
app.controller('DanaDesaCtrl', ['$scope', 'dana_desa', '$modal', '$stateParams', function ($scope, dana_desa, $modal) {

    $scope.prev = "";
    $scope.dtorganisasi = {};
    $scope.openModalOrganisasi = function () {
        var modalInstance = $modal.open({
            templateUrl: 'modOrganisasi.html',
            controller: 'ModalOrganisasiCtrl',
            size: 'lg',
            backdrop: 'static',
            resolve: {
                dtorganisasi: function () {
                    return $scope.dtorganisasi;
                }
            }
        });

        modalInstance.result.then(function (selectedItem) {
            $scope.dtorganisasi = selectedItem;
            $scope.prev = $scope.dtorganisasi.desa;
        }, function () {
    ...
    //init get data
    dana_desa.get($scope.main.page, $scope.main.term)
        .success(function (data) {
            //Hide loading
...
// button add on click
    $scope.add = function () {
        ...
        //dana_desa.getorganisasi()
        //    .success(function (data) {
        //        if (data.length == 0) {
        //            $scope.alertset = {
        //                class: 'warning'
        //            };
        //            $scope.success = 'Data Pemerintah Desa kosong!';
        //            $scope.done();
        //
        //            $scope.disConUmum(false);
        //            $scope.disConAksi(false);
        ...
        //        $scope.redirect(data);
        //    });
        dana_desa.getsumberdana()
            .success(function (data) {
                //$scope.objOrganisasi = data;
                //$scope.inputData.organisasi = $scope.objOrganisasi[0];

                $scope.objSumberDana = data;
    ...
    $scope.clearInput = function () {
        ...
        $scope.main.page = 1;
        $scope.inputData._id = '';
        $scope.inputData.organisasi_id = '';
        $scope.inputData.sumber_dana_id = '';
...
// edit data yang akan ditampilkan ke form
    $scope.edit = function (_id) {
        ...
        dana_desa.show(_id)
            .success(function (data) {
                dana_desa.getDetilOrganisasi(data.organisasi_id)
                    .success(function (data1) {
                        dana_desa.getsumberdana()
                            .success(function (data2) {
                                $scope.dtorganisasi = data1;
                                $scope.prev = $scope.dtorganisasi.desa;
                                $scope.objSumberDana = data2;
...
// update data
    $scope.updateData = function () {
        ...
        if ($scope.editForm.$valid) {
            ...
            $scope.inputData.sumber_dana_id = $scope.inputData.sumber_dana._id;
            $scope.inputData._token = $scope.csrf;
            dana_desa.update($scope.inputData)
                .success(function (data) {
                    if (data.success == true) {
                        $scope.alertset = {
                            class: 'success'
